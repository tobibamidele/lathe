...
// DefaultFunc supplies the column's value in Go, on every insert, by calling a
// package-level function in the schema package. It is the type-checked sibling
// of [ColumnBuilder.Default]: f must return the column's Go type (a string for
// Text and VarChar, uuid.UUID for UUID, int64 for BigInt, and so on), so a
// mismatched default is a compile error.
//
//	f := generateSomeID
//	s.Text("id").PrimaryKey().DefaultFunc(f)
//
// Unlike Default and DefaultNow, DefaultFunc declares no server-side default:
// the column has none in SQL. Generated code calls the function per insert when
// the field still holds its zero value, before the INSERT statement is built -
// the function can produce a fresh random value every row. To make that call,
// the generated package imports the schema package, so f must be exported
// (GenerateSomeID, not generateSomeID) and a plain package-level function:
// closures and method values have no name the generated code can call.
//
// A column has either a server-side default or a DefaultFunc, whichever was
// called last. For a random primary key prefer:
//
//	s.UUID("id").PrimaryKey().DefaultFunc(uuid.NewString)
//
// or Database's per-row [DefaultUUID].
func (c *ColumnBuilder[V]) DefaultFunc(f func() V) *ColumnBuilder[V] {
	name := runtime.FuncForPC(reflect.ValueOf(f).Pointer()).Name()
	leaf := name[strings.LastIndex(name, ".")+1:]
	switch {
	case name == "" || !strings.Contains(name, ".") || strings.HasSuffix(name, ".<autogenerated>"):
		c.buildErr = append(c.buildErr, fmt.Errorf("column %q: DefaultFunc could not resolve the function to a named package-level function", c.col.Name))
		return c
	case strings.Contains(name, "(") || strings.HasSuffix(name, "-fm"):
		c.buildErr = append(c.buildErr, fmt.Errorf("column %q: DefaultFunc must be a package-level function, not a method value", c.col.Name))
		return c
	case strings.Contains(name, ".func"):
		c.buildErr = append(c.buildErr, fmt.Errorf("column %q: DefaultFunc must be a named package-level function, not a closure", c.col.Name))
		return c
	case leaf == "" || !unicode.IsUpper(rune(leaf[0])):
		c.buildErr = append(c.buildErr, fmt.Errorf("column %q: DefaultFunc %s is not exported; rename it so the generated package can call it", c.col.Name, name))
		return c
	}
	c.col.Default = nil
	c.col.DefaultFunc = name
	return c
}
