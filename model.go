...
// Load fills in relations of a row you already have.
func (m *Model[M]) Load(ctx context.Context, row *M, rels ...Preloader[M]) error {
	return runPreloads(ctx, m.db, []*M{row}, rels)
}

// LoadMany fills in relations of rows you already have, in batches.
func (m *Model[M]) LoadMany(ctx context.Context, rows []M, rels ...Preloader[M]) error {
	ptrs := make([]*M, len(rows))
	for i := range rows {
		ptrs[i] = &rows[i]
	}
	return runPreloads(ctx, m.db, ptrs, rels)
}
...
// With loads relations of the returned rows after the main query, in batches:
// one extra query per relation, however many rows came back. See [Relation].
func (q *FindQuery[M]) With(rels ...Preloader[M]) *FindQuery[M] {
	q.rels = append(q.rels, rels...)
	return q
}
...
func (q *FindQuery[M]) All(ctx context.Context) ([]M, error) {
	...
	if err := checkRequirements(q.m, idx, q.rels); err != nil {
		return nil, err
	}
	...
	return out, q.preload(ctx, out)
}

func (q *FindQuery[M]) preload(ctx context.Context, rows []M) error {
	if len(q.rels) == 0 || len(rows) == 0 {
		return nil
	}
	ptrs := make([]*M, len(rows))
	for i := range rows {
	...
	return runPreloads(ctx, q.m.db, ptrs, q.rels)
}
...
func (q *FindQuery[M]) One(ctx context.Context) (*M, error) {
	...
	if err := q.preload(ctx, out); err != nil {
		return nil, err
	}
	return &out[0], nil
}
...
// insertBuilder renders INSERT INTO t (cols) VALUES (...) for row, without any
// RETURNING or conflict clause.
func (m *Model[M]) insertBuilder(row *M) (*builder, insertPlan) {
	d := m.db.dialect
	p := m.planInsert(row)
	b := newBuilder(d)
	if len(p.cols) == 0 {
...
// checkReadBack refuses inserts whose primary key the database would generate
// but this dialect cannot return.
func (m *Model[M]) checkReadBack(p insertPlan) error {
	for _, di := range p.defaults {
		if m.spec.Fields[di].PrimaryKey {
			return fmt.Errorf("lathe: %s.%s is a primary key generated by a database default, which %s cannot read back after INSERT; assign it in Go",
				m.spec.Table, m.spec.Fields[di].Column, m.db.dialect.Name())
		}
	}
	return nil
}
